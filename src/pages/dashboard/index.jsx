...
const DashboardPage = () => {
  ...
  const companyName = authUser?.companyName || authUser?.company?.name || authUser?.storeName || authUser?.name || "Sir";
  ...
  const filteredSalesData = useMemo(() => {
    ...
    return (Array.isArray(apiData) ? apiData : []).map((d) => {
      const name =
        d?.name ?? d?.label ?? d?.month ?? d?.date ?? d?.period ?? "";
      const raw =
        d?.totalPNL ??
        d?.revenue ??
        d?.value ??
  ...
  const currentSubscriberData = useMemo(() => {
    ...
    return apiData.map((d) => {
      const name =
        d?.name ?? d?.label ?? d?.date ?? d?.week ?? d?.month ?? "";
      const out = { name };
      Object.keys(d || {}).forEach((k) => {
        if (["name", "label", "date", "week", "month"].includes(k)) return;
        const raw = d[k];
        const num =
          typeof raw === "number"
            ? raw
      ...
      if (out.value == null) {
        const raw = d?.value ?? d?.count ?? d?.total;
        const num =
          typeof raw === "number"
            ? raw
            : Number(String(raw ?? "").replace(/,/g, ""));
        if (!Number.isNaN(num)) out.value = num;
      }
      return out;
    });
