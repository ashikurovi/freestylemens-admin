...
export default function SalesOverviewChart({
  ...
  const seriesKeys = Array.isArray(data)
    ? Object.keys(data.find((d) => d && typeof d === "object") || {}).filter(
        (k) => {
          if (["name", "date", "label", "period", "month"].includes(k)) {
            return false;
          }
          return data.some((d) => typeof (d && d[k]) === "number");
        },
      )
    : [];
        ...
        <div className="h-[300px] w-full mt-4">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} barSize={20}>
              ...
              {(seriesKeys.length ? seriesKeys : ["Revenue"]).map(
                (key, idx) => (
                  <Bar
                    key={key}
                    dataKey={key}
